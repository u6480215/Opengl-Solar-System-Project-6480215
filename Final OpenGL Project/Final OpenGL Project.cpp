...
// Create Frame Buffer Object
   ...
   PlanetParams mars(30.0f, 24.13f, 360.0f / 24.6f, 25.2f, 0.093f, 1.9f, 0.7f, marsTexture, 0.0008f, 0.1f, 8.0f, glm::vec3(0.5f, 0.45f, 0.45f), 2.02f, glm::vec3(0.055f, 0.015f, 0.0f) + 0.02f, 5.9f);
   PlanetParams jupiter(55.0f, 13.07, 360.0f / 9.9f, 3.1f, 0.049f, 1.3f, 3.5f, jupiterTexture, 0.0007f, 0.06f, 5.0f, glm::vec3(0.55f, 0.54f, 0.5f), 1.03f, glm::vec3(0.02f, 0.0076f, 0.004f) + 0.02f, 5.9f, glm::vec3(-0.07f, -0.01f, -0.00f), 4.6f);
   PlanetParams saturn(65.0f, 9.69f, 360.0f / 10.7f, 26.7f, 0.056f, 2.5f, 2.0f, saturnTexture, 0.0006f, 0.05f, 5.0f, glm::vec3(0.54f, 0.57f, 0.5f), 1.03f, glm::vec3(0.015f, 0.0076f, 0.006f) + 0.02f, 5.9f, glm::vec3(-0.12f, -0.057f, -0.002f), 3.5f);
   PlanetParams uranus(75.0f, 6.81f, 360.0f / 17.2f, 97.8f, 0.046f, 0.8f, 1.5f, uranusTexture, 0.0005f, 0.04f, 5.0f, glm::vec3(0.7f, 0.7f, 0.8f), 0.73f, glm::vec3(0.021f, 0.025f, 0.026f) + 0.02f, 5.5f, glm::vec3(-0.008f, -0.04f, -0.0f), 3.5f);
   PlanetParams neptune(85.0f, 5.43f, 360.0f / 16.1f, 28.3f, 0.046f, 0.8f, 1.5f, neptuneTexture, 0.0004f, 0.04f, 8.0f, glm::vec3(0.5f, 0.5f, 0.9f), 0.22f, glm::vec3(0.001f, 0.001f, 0.017f) + 0.02f, 5.9f, glm::vec3(-0.0f, -0.027f, -0.0f), 3.5f);
   PlanetParams pluto(110.0f, 4.67f, 360.0f / 153.3f, 122.5f, 0.248f, 17.2f, 0.4f, plutoTexture, 0.0003f, 0.02f, 5.0f, glm::vec3(0.5f, 0.5f, 0.5f), 0.92f, glm::vec3(0.01f, 0.01f, 0.01f) + 0.02f, 5.9f);
   PlanetParams moon(2.5f, 1.022f * 50, 360.0f / 27.3f, 6.68f, 0.0549f, 5.145f, 0.27f, moonTexture, 0.00087f, 0.1f, 32.0f, glm::vec3(0.4f, 0.4f, 0.4f), 2.95f, glm::vec3(0.02f, 0.0015f, 0.005f) + 0.02f, 2.9f);
   ...
   std::vector<std::reference_wrapper<Orbit>> orbits = { mercuryOrbit, venusOrbit, earthOrbit, moonOrbit, marsOrbit, jupiterOrbit, saturnOrbit, uranusOrbit, neptuneOrbit, plutoOrbit };
    GLuint saturnsRing = createRingVAO(saturn.scale + 0.5,1.4f);
    GLuint uranusRing = createRingVAO(uranus.scale + 0.0, uranus.scale+0.57f);
...
//------------------------------------------ ASTEROIDS ----------------------------------------------
    ...
    std::vector<glm::mat4> asteroidTransforms = asteroids(38.5,6.5,0.2f, 0.15f,1.08f,1.1,-1.1);
...
// Create a buffer for instance matrices
    ...
    glBindVertexArray(asteroid); // Assuming the sphere VAO represents the asteroid model
    ...
    {   
        ...
        glm::mat4 projection = glm::perspective(glm::radians(fov), (float)SCR_WIDTH / (float)SCR_HEIGHT, 0.1f, 1000.0f);
            ...
            orbitShader.setFloat("gamma", gamma);// Render each orbit       
            ringShader.use();
