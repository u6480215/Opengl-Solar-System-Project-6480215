...
void main() {
    ...
    if (flipped) Normal *= -1;
    vec4 fullTexture = texture(texture1, TexCoords); // Sample the texture
    // Optional: Handle transparency by discarding transparent fragments
    if (fullTexture.a < 0.1)  // Change threshold as needed
        discard;
    ...
    vec3 textureColor = fullTexture.rgb;
